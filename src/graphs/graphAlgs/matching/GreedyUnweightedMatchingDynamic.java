...
public class GreedyUnweightedMatchingDynamic extends MatchingAlgorithm {
	PriorityQueue<Vertex<java.awt.geom.Point2D.Double>> vertices = new PriorityQueue<Vertex<java.awt.geom.Point2D.Double>>(
	...
	public void runAlgorithm(AdjacencyListGraph<Point2D.Double, Double> g) {
		...
		while (vertices.isEmpty() != true) {
			...
			PriorityQueue<Vertex<java.awt.geom.Point2D.Double>> neighbor_list = new PriorityQueue<Vertex<java.awt.geom.Point2D.Double>>(
					new VertexDegreeComparator());
			for (Edge currEdge : g.incidentEdges(least_value)) {
				Vertex<java.awt.geom.Point2D.Double> endpt = g.opposite(least_value, currEdge);
				if (endpt.get(VERTEX_STATUS) == FREE) {
					neighbor_list.add(endpt);
				}
			}
			if (neighbor_list.isEmpty() == false) {
				Vertex<java.awt.geom.Point2D.Double> neighbor = neighbor_list.poll();
				for (Edge currEdge : g.edges()) {
					...
					if (endpts[0].equals(least_value) && endpts[1].equals(neighbor)) {
						...
						for (Edge neighbors_neighbor : g.incidentEdges(endpts[1])) {
							Vertex<java.awt.geom.Point2D.Double> endpt = g.opposite(neighbor, neighbors_neighbor);
									if (endpt.get(VERTEX_STATUS).equals(FREE))
									{
										endpt.put(DEGREE, g.degree(endpt)-1);
									}
						}
